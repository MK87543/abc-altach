...
-- ==============================================================================
-- ANLEITUNG ZUR AUSFÜHRUNG IM SUPABASE DASHBOARD:
-- 1. Öffne dein Supabase Dashboard: https://supabase.com/dashboard/project/mbiacrtcfiuxkiybrolx
-- 2. Klicke links im Menü auf "SQL Editor".
-- 3. Klicke auf "New query" (+).
-- 4. Kopiere den gesamten Inhalt dieser Datei hinein.
-- 5. Klicke unten rechts auf "Run" (oder drücke Strg+Enter).
--
-- WICHTIGER HINWEIS:
-- Dieses Skript löscht KEINE vorhandenen Daten! Es aktiviert RLS, richtet saubere
-- Zugriffsrechte für alle Trainer ein und erstellt die Tabelle für Abwesenheiten.
-- ==============================================================================
...
CREATE TABLE IF NOT EXISTS public.coach_absences (
    ...
    recurrence_interval INTEGER DEFAULT 1 CHECK (recurrence_interval >= 1), -- Intervall in Wochen (1 = wöchentlich, 2 = alle 2 Wochen, etc.)
    reason TEXT, -- z.B. "Urlaub", "Beruflich", "Jeden Dienstag keine Zeit"
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Falls Tabelle bereits existierte, neue Spalten sicher nachrüsten
ALTER TABLE public.coach_absences ADD COLUMN IF NOT EXISTS recurring_days INTEGER[] DEFAULT '{}';
ALTER TABLE public.coach_absences ADD COLUMN IF NOT EXISTS recurrence_interval INTEGER DEFAULT 1;
